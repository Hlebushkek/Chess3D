...
public abstract class ChessBoardAbstract : MonoBehaviour
{
    ...
    [SerializeField] protected Material moveHighlightM, attackHighlightM;
    ...
    {
        ClearHighlight();
        Highlight(piece.transform.position, piece.GetMoveOffsets(), piece.GetAttackOffsets(), team);
    }
    protected void Highlight(Vector3 piecePos, Vector2[][] moveOffsets, Vector2[][] attackOffsets, Team team)
    {
        piecesController = FindObjectOfType<PiecesController>();
        ...
        foreach (Vector2[] offsetsLine in moveOffsets)
        {
            foreach (Vector2 offset in offsetsLine)
            {
                ...
                if (x >= 0 && z >= 0 && x < 8 && z < 8)
                {
                    ...
                    boardCells[x, z].GetComponent<MeshRenderer>().material = moveHighlightM;
        ...
        if (attackOffsets == null) return;
        foreach (Vector2[] offsetsLine in attackOffsets)
        {
            foreach (Vector2 offset in offsetsLine)
            {
                int x = 0;
                ...
                {
                    //Debug.Log(x +";"+ z + "   " + boardCells[x,z].isEmpty());
                    if (boardCells[x,z].isOccupied() && boardCells[x,z].getOccupiedTeam() != team)
                    {
                        boardCells[x, z].GetComponent<MeshRenderer>().material = attackHighlightM;
                        highlightedCells.Add(new Vector2Int(x, z));
                    }
    ...
    {
        if (team == Team.White) Debug.LogWarning("Black Team Win");
        else if (team == Team.Black) Debug.LogWarning("White Team Win");
    }
}
